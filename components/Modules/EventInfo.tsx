...
const EventInfo: React.FC<EventInfoProps> = ({ data, onBack }) => {
  ...
  const filteredEvents = useMemo(() => {
    return data.events.filter(e => {
      const eCatRaw = (e.category || 'General').trim();
      const eCat = eCatRaw.toLowerCase();
      const aCat = activeCat.trim().toLowerCase();
      
      // If we are looking for a specific branch (Comp, IT, etc.)
      if (aCat !== 'general') {
        return eCat === aCat;
      }
      
      // If we are on the General tab
      if (aCat === 'general') {
        // Explicitly general
        if (eCat === 'general') return true;
        
        // OR it's not one of our known specific branches (fallback)
        const specificBranchSlugs = categories.slice(1).map(c => c.toLowerCase());
        const isKnownBranch = specificBranchSlugs.includes(eCat);
        ...
        {categories.map(cat => (
          <button key={cat} onClick={() => setActiveCat(cat)}
            className={`px-5 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap
              ${activeCat === cat ? 'bg-pink-600 text-white shadow-lg' : 'bg-white dark:bg-slate-900 text-slate-400 shadow-sm border border-slate-100 dark:border-slate-800'}`}>
            {cat}
          </button>
        ))}
      </div>
            ...
            <div key={ev.id} className="bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-sm border border-pink-50 dark:border-slate-800 animate-slideUp" style={{ animationDelay: `${i * 100}ms` }}>
              ...
              <div className="p-6">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[8px] font-black text-pink-500 bg-pink-50 dark:bg-pink-900/20 px-2 py-1 rounded-full uppercase tracking-widest">{ev.category || 'General'}</span>
